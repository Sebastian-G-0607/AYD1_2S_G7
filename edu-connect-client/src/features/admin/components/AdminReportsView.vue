    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-4">
      <div>
        <h1
          ...
          style="font-size: 32px; font-weight: 700; letter-spacing: -0.02em; line-height: 1.2; color: rgb(30, 41, 59);"
        >
      ...
      <div class="flex flex-wrap items-center gap-4 bg-surface-container-lowest p-2 rounded-xl shadow-sm">
        <!-- Selector de Período -->
        <div class="relative">
          <button
            type="button"
          ...
          >
            <span class="material-symbols-outlined text-on-surface-variant group-hover:text-primary transition-colors text-[20px]">
              calendar_month
            </span>
            <span class="font-label-md text-label-md text-on-surface">
              {{ periodOptions.find((p) => p.value === selectedPeriod)?.label || 'Últimos 30 días' }}
            </span>
            <span class="material-symbols-outlined text-on-surface-variant text-[18px] transition-transform" :class="{ 'rotate-180': isPeriodDropdownOpen }">
              arrow_drop_down
            </span>
          </button>
          ...
          >
            <button
              ...
              :class="selectedPeriod === opt.value ? 'text-primary font-semibold bg-surface-container-low' : 'text-on-surface'"
              @click="selectPeriod(opt.value as any)"
            >
              {{ opt.label }}
              <span v-if="selectedPeriod === opt.value" class="material-symbols-outlined text-[18px] text-primary">check</span>
            </button>
          </div>
        </div>
      ...
      <div class="bg-surface-container-lowest rounded-2xl p-6 shadow-sm hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden group">
        ...
        <div class="flex justify-between items-start mb-4 relative z-10">
          <div class="w-12 h-12 bg-primary-container rounded-xl flex items-center justify-center">
            <span class="material-symbols-outlined text-on-primary-container text-[24px]" style="font-variation-settings: 'FILL' 1;">
              groups
            </span>
          </div>
          <div class="px-2 py-1 bg-surface-container-low rounded text-on-surface font-label-sm text-label-sm flex items-center gap-1">
            <span class="material-symbols-outlined text-[14px] text-on-surface-variant">trending_up</span> Activo
          </div>
        </div>
        <div class="relative z-10">
          <p class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider mb-1">
      ...
      <div class="bg-surface-container-lowest rounded-2xl p-6 shadow-sm hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden group">
        ...
        <div class="flex justify-between items-start mb-4 relative z-10">
          <div class="w-12 h-12 bg-secondary-container rounded-xl flex items-center justify-center">
            <span class="material-symbols-outlined text-on-secondary-container text-[24px]" style="font-variation-settings: 'FILL' 1;">
              badge
            </span>
          </div>
          <div class="px-2 py-1 bg-surface-container-low rounded text-on-surface font-label-sm text-label-sm flex items-center gap-1">
            <span class="material-symbols-outlined text-[14px] text-on-surface-variant">trending_up</span> Top
          </div>
        </div>
        <div class="relative z-10">
          <p class="font-label-md text-label-md text-on-surface-variant uppercase tracking-wider mb-1">
      ...
      <div class="bg-surface-container-lowest rounded-2xl p-6 shadow-sm hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden group">
        ...
        <div class="flex justify-between items-start mb-4 relative z-10">
          <div class="w-12 h-12 bg-tertiary-container rounded-xl flex items-center justify-center">
            <span class="material-symbols-outlined text-on-tertiary-container text-[24px]" style="font-variation-settings: 'FILL' 1;">
              school
            </span>
          </div>
          <div class="px-2 py-1 bg-surface-container-low rounded text-on-surface font-label-sm text-label-sm flex items-center gap-1">
            <span class="material-symbols-outlined text-[14px] text-on-surface-variant">trending_up</span>
      ...
      <div class="bg-primary rounded-2xl p-6 shadow-md hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden group">
        ...
        <div class="flex justify-between items-start mb-4 relative z-10">
          <div class="w-12 h-12 bg-primary-fixed/20 rounded-xl flex items-center justify-center backdrop-blur-sm">
            <span class="material-symbols-outlined text-on-primary text-[24px]" style="font-variation-settings: 'FILL' 1;">
              star
            </span>
          </div>
        </div>
      ...
      <div class="lg:col-span-2 bg-surface-container-lowest rounded-2xl p-6 shadow-sm flex flex-col justify-between">
        ...
        >
          ...
          <div
            ...
            :style="{ height: `${Math.max(12, Math.round((tutor.totalSesionesAtendidas / maxAtencionesTutor) * 88))}%` }"
          >
        ...
        >
          ...
          <p class="font-body-sm text-body-sm text-on-surface-variant">Las sesiones deben estar en estado ATENDIDA para figurar en este reporte.</p>
        </div>
      </div>
        ...
        <div class="relative w-48 h-48 mx-auto mb-6 flex-shrink-0">
          ...
          <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span class="font-headline-md text-headline-md text-on-surface">
              {{ resumen.totalSesiones > 999 ? (resumen.totalSesiones / 1000).toFixed(1) + 'k' : resumen.totalSesiones }}
            </span>
        ...
        <div class="space-y-2 mt-auto">
          ...
          >
            <div class="flex items-center gap-3 min-w-0">
              ...
              <span class="font-label-md text-label-md text-on-surface truncate group-hover:text-primary">
                {{ seg.nombreMateria }}
              </span>
            </div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-surface-container-high pb-4 mb-6">
        <div class="flex items-center gap-2">
          <button
            ...
            :class="selectedTab === 'todos' ? 'bg-primary text-on-primary shadow-sm font-semibold' : 'bg-surface-container-low text-on-surface-variant hover:bg-surface-container hover:text-on-surface'"
          ...
          <button
            type="button"
            class="font-label-sm text-label-sm px-4 py-2 rounded-full transition-all cursor-pointer"
            :class="selectedTab === 'tutores' ? 'bg-primary text-on-primary shadow-sm font-semibold' : 'bg-surface-container-low text-on-surface-variant hover:bg-surface-container hover:text-on-surface'"
            @click="selectedTab = 'tutores'"
          >
          ...
          <button
            type="button"
            class="font-label-sm text-label-sm px-4 py-2 rounded-full transition-all cursor-pointer"
            :class="selectedTab === 'materias' ? 'bg-primary text-on-primary shadow-sm font-semibold' : 'bg-surface-container-low text-on-surface-variant hover:bg-surface-container hover:text-on-surface'"
            @click="selectedTab = 'materias'"
          >
      ...
      <div v-if="selectedTab === 'todos' || selectedTab === 'tutores'" class="mb-8">
        ...
        <div class="overflow-x-auto rounded-xl border border-surface-container-high">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-surface-container-low text-on-surface-variant font-label-sm text-label-sm border-b border-surface-container-high">
            ...
            <tbody class="divide-y divide-surface-container-high font-body-sm text-body-sm">
              ...
              >
                ...
                <td class="py-3 px-4">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-primary-container text-on-primary-container flex items-center justify-center font-bold text-xs overflow-hidden shrink-0">
                      <img
